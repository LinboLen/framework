...
func (s *QueriesRelationshipsTestSuite) TestHas_Existence() {
	for driver, query := range s.queries {
		s.Run(driver, func() {
			alice := &User{Name: "rel_has_alice", Books: []*Book{{Name: "ab1"}, {Name: "ab2"}}}
			s.Nil(query.Query().Select(orm.Associations).Create(&alice))
			bob := &User{Name: "rel_has_bob"}
			s.Nil(query.Query().Create(&bob))
			carol := &User{Name: "rel_has_carol", Books: []*Book{{Name: "cb1"}}}
			s.Nil(query.Query().Select(orm.Associations).Create(&carol))
...
func (s *QueriesRelationshipsTestSuite) TestHas_CountComparison() {
	for driver, query := range s.queries {
		s.Run(driver, func() {
			alice := &User{Name: "rel_hasc_alice", Books: []*Book{{Name: "h1"}, {Name: "h2"}, {Name: "h3"}}}
			s.Nil(query.Query().Select(orm.Associations).Create(&alice))
			bob := &User{Name: "rel_hasc_bob", Books: []*Book{{Name: "h4"}}}
...
func (s *QueriesRelationshipsTestSuite) TestDoesntHave() {
	for driver, query := range s.queries {
		s.Run(driver, func() {
			withBooks := &User{Name: "rel_dh_with", Books: []*Book{{Name: "dhb"}}}
...
func (s *QueriesRelationshipsTestSuite) TestWhereHas_Callback() {
	for driver, query := range s.queries {
		s.Run(driver, func() {
			match := &User{Name: "rel_wh_match", Books: []*Book{{Name: "wh_target"}}}
			s.Nil(query.Query().Select(orm.Associations).Create(&match))
			other := &User{Name: "rel_wh_other", Books: []*Book{{Name: "wh_other"}}}
			s.Nil(query.Query().Select(orm.Associations).Create(&other))
...
func (s *QueriesRelationshipsTestSuite) TestHas_BelongsTo() {
	for driver, query := range s.queries {
		s.Run(driver, func() {
			user := &User{Name: "rel_bt_user", Address: &Address{Name: "rel_bt_address"}}
...
func (s *QueriesRelationshipsTestSuite) TestHas_ManyToMany() {
	for driver, query := range s.queries {
		s.Run(driver, func() {
			withRole := &User{Name: "rel_mtm_with", Roles: []*Role{{Name: "rel_mtm_role"}}}
			s.Nil(query.Query().Select(orm.Associations).Create(&withRole))
...
func (s *QueriesRelationshipsTestSuite) TestHasMorph() {
	for driver, query := range s.queries {
		s.Run(driver, func() {
			withHouse := &User{Name: "rel_hm_with", House: &House{Name: "rel_hm_house"}}
			s.Nil(query.Query().Select(orm.Associations).Create(&withHouse))
...
func (s *QueriesRelationshipsTestSuite) TestNestedHas() {
	for driver, query := range s.queries {
		s.Run(driver, func() {
			...
			carol := &User{
				Name: "rel_nested_carol",
				Books: []*Book{
					{Name: "carol_book", Author: &Author{Name: "carol_author"}},
				},
			}
			s.Nil(query.Query().Select(orm.Associations).Create(&carol))
			dan := &User{Name: "rel_nested_dan", Books: []*Book{{Name: "dan_book"}}}
			s.Nil(query.Query().Select(orm.Associations).Create(&dan))
...
func (s *QueriesRelationshipsTestSuite) TestHasManyThrough_WhereHas() {
	for driver, query := range s.queries {
		s.Run(driver, func() {
			...
			u1 := &User{Name: "rel_th_u1", Books: []*Book{{Name: "th_book1", Author: &Author{Name: "th_author_match"}}}}
			s.Nil(query.Query().Select(orm.Associations).Create(&u1))
			u2 := &User{Name: "rel_th_u2", Books: []*Book{{Name: "th_book2"}}}
