...
var (
	...
	AIProviderDoesNotSupportAudio         = New("ai: provider %s does not support audio")
	AIProviderDoesNotSupportFiles         = New("ai: provider %s does not support files")
	AIProviderDoesNotSupportImages        = New("ai: provider %s does not support images")
	AIAudioNameRequired                   = New("ai: audio name is required")
	AIAudioPromptRequired                 = New("ai: audio prompt is required")
	AIAudioResponseIsEmpty                = New("ai: audio response is empty")
	AIAudioStorePathInvalid               = New("ai: audio store path is invalid")
	AIAudioStorePathRequired              = New("ai: audio store path is required")
	AIAudioStoreTooManyPaths              = New("ai: audio store accepts at most one disk")
	...
	AIImageStorePathInvalid               = New("ai: image store path is invalid")
	...
	AIProviderDoesNotSupportTranscription = New("ai: provider %s does not support transcription")
	...
	AITranscriptionFileRequired           = New("ai: transcription file is required")
	AITranscriptionResponseIsEmpty        = New("ai: transcription response is empty")
	...
	OrmRelationThroughNotConfigured = New("through relation %q must be declared via the Relations() method on model %s")
	OrmMorphRelationNotConfigured   = New("polymorphic relation %q must be declared via the Relations() method on model %s")
	OrmMorphRelationKindUnknown     = New("relation %q on model %s has unknown kind %q")
	OrmMorphRelationMissingField    = New("relation %q on model %s is missing required field %q")
	...
	OrmPolymorphicTagForbidden      = New("polymorphic GORM tag on field %q of model %s is forbidden; declare the relation via the Relations() method instead")
	OrmRelationTagForbidden         = New("GORM relation tag on field %q of model %s is forbidden; declare the relation via the Relations() method instead, and tag the field with `gorm:\"-\"`")
	OrmRelationParentNotPointer     = New("Related: parent must be a non-nil pointer to a struct, got %T")
	...
	OrmRelationPivotFieldNotStruct  = New("eager-load: related model %s has %s field of kind %s; pivot hydration target must be a struct")
