...
// NewRelation is the public Query-level entry point. The Orm-level entry (Orm.NewRelation)
// delegates here. Returns a fresh Query pre-scoped to the related rows for parent.relation.
func (r *Query) NewRelation(parent any, relation string) contractsorm.Query {
...
// / Get / etc. on the returned Query. Write operations live on Orm directly (Save, Attach, ...).
//
// Public entry: Orm.NewRelation delegates here on a fresh-session *Query.
func (r *Query) newRelationQuery(parent any, relation string) contractsorm.Query {
	if !isValidParent(parent) {
		return r.guardedQuery(errors.OrmNewRelationParentNotPointer.Args(parent))
	}
