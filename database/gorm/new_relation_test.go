...
func TestNewRelation_HasMany_Where(t *testing.T) {
	q := newRelQueryWith(t, &relUser{})
	rel := q.NewRelation(&relUser{ID: 7}, "Books")
	sql := newRelationSQL(t, rel, &[]relBook{})
	assert.Contains(t, sql, "rel_books")
	assert.Contains(t, sql, "user_id")
}

func TestNewRelation_BelongsTo_Where(t *testing.T) {
	q := newRelQueryWith(t, &relBook{})
	rel := q.NewRelation(&relBook{AuthorID: 5}, "Author")
...
func TestNewRelation_MorphMany_AddsTypeFilter(t *testing.T) {
	q := newRelQueryWith(t, &relUser{})
	rel := q.NewRelation(&relUser{ID: 9}, "Houses")
	sql := newRelationSQL(t, rel, &[]relHouse{})
	assert.Contains(t, sql, "houseable_id")
	assert.Contains(t, sql, "houseable_type")
}

func TestNewRelation_MorphOne_AddsTypeFilter(t *testing.T) {
	q := newRelQueryWith(t, &relUser{})
	rel := q.NewRelation(&relUser{ID: 9}, "Logo")
	sql := newRelationSQL(t, rel, &relLogo{})
	assert.Contains(t, sql, "logoable_id")
	assert.Contains(t, sql, "logoable_type")
}

func TestNewRelation_HasManyThrough(t *testing.T) {
	q := newRelQueryWith(t, &relCountry{})
	rel := q.NewRelation(&relCountry{ID: 1}, "Posts")
...
func TestNewRelation_NotPointer(t *testing.T) {
	q := newRelQueryWith(t, &relUser{})
	rel := q.NewRelation(relUser{ID: 1}, "Books") // value, not pointer
	gq, ok := rel.(*Query)
	assert.True(t, ok)
	assert.True(t, errors.Is(gq.instance.Error, errors.OrmNewRelationParentNotPointer))
}

func TestNewRelation_NilParent(t *testing.T) {
	q := newRelQueryWith(t, &relUser{})
	rel := q.NewRelation(nil, "Books")
	gq, ok := rel.(*Query)
	assert.True(t, ok)
	assert.True(t, errors.Is(gq.instance.Error, errors.OrmNewRelationParentNotPointer))
}

func TestNewRelation_RelationNotFound(t *testing.T) {
	q := newRelQueryWith(t, &relUser{})
	rel := q.NewRelation(&relUser{ID: 1}, "DoesNotExist")
	gq, ok := rel.(*Query)
	assert.True(t, ok)
...
func TestNewRelation_MorphTo_ResolvedViaMorphMap(t *testing.T) {
	morphmap.Reset()
	defer morphmap.Reset()
	morphmap.Register(map[string]any{"post": &morphParentLikePost{}})

	q := newRelQueryWith(t, &morphImage{})
	rel := q.NewRelation(&morphImage{ID: 1, ImageableID: 42, ImageableType: "post"}, "Imageable")
	sql := newRelationSQL(t, rel, &morphParentLikePost{})
	assert.Contains(t, sql, "morph_parent_like_posts")
	assert.Contains(t, sql, "id")
}

func TestNewRelation_MorphTo_UnregisteredType(t *testing.T) {
	morphmap.Reset()
	defer morphmap.Reset()

	q := newRelQueryWith(t, &morphImage{})
	rel := q.NewRelation(&morphImage{ID: 1, ImageableID: 42, ImageableType: "unknown"}, "Imageable")
	gq, ok := rel.(*Query)
	assert.True(t, ok)
	assert.True(t, errors.Is(gq.instance.Error, errors.OrmMorphTypeUnknown))
}

func TestNewRelation_MorphTo_EmptyType_YieldsZeroRowQuery(t *testing.T) {
	morphmap.Reset()
	defer morphmap.Reset()

	q := newRelQueryWith(t, &morphImage{})
	rel := q.NewRelation(&morphImage{ID: 1, ImageableID: 0, ImageableType: ""}, "Imageable")
	gq, ok := rel.(*Query)
	assert.True(t, ok)
...
func TestNewRelation_MorphToMany_AddsPivotTypeFilter(t *testing.T) {
	q := newRelQueryWith(t, &morphPost{})
	rel := q.NewRelation(&morphPost{ID: 3}, "Tags")
